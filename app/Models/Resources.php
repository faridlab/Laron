...
class Resources extends Model
{
  ...
  public function getStructure() {
    $not_displayed = array("id", "created_at", "updated_at", "deleted_at");
  ...
  public function search($request) {
    ...
    if(!empty(trim($request->input('search.value')))) {
        foreach ($request->get('columns') as $key => $value) {
            if($value['searchable']) {
                $query->orWhere($value['data'], 'LIKE', '%' . trim($request->input('search.value')) . '%');
            }
        }
    }
    ...
    if($request->get('order')) {
        foreach ($request->get('order') as $key => $value) {
            $query->orderBy($request->get('columns')[$value['column']]['data'], $value['dir']);
        }
    }
